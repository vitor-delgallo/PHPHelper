...
final class SQLTest extends TestCase {
    ...
    public static function invalidAutoDerivedColumnProvider(): array {
        return [
            ...
            'interior newline'  => ["col\nname"],
            // The case the provider used to miss. "col\nname" is rejected by an anchored regex too,
            // so it proved nothing about the anchors; only a newline in TRAILING position does.
            'trailing newline'  => ["col\n"],
            'trailing crlf'     => ["col\r\n"],
            'leading newline'   => ["\ncol"],
            'only a newline'    => ["\n"],
            'null byte'         => ["col\0"],
    ...
    /**
     * FINDING (fixed): the identifier check was '/^[A-Za-z0-9_$]+$/', and PCRE's '$' matches BEFORE
     * A FINAL NEWLINE. So "col\n" satisfied the one pattern standing between an attacker-chosen
     * array key and a raw-interpolated identifier, and the docblock's "must match or THROW" was
     * false. The commit that introduced this check is the same one that documented and closed the
     * identical '$' hole in Validator::isOctal — the provider's only newline case was "col\nname",
     * whose newline is INTERIOR and which an anchored pattern rejects anyway, so it never touched
     * the anchor.
     *
     * Pinned as its own test, not just a provider row, because it is the exact byte that got
     * through: before the fix this call returned
     * "INSERT INTO t (`col\n`) VALUES ('v') ON DUPLICATE KEY UPDATE `col\n`=VALUES(`col\n`)"
     * instead of throwing.
     */
    public function testPrepareRejectsAutoDerivedColumnNameWithATrailingNewlineThePatternAnchorLetThrough(): void {
        $data = [["col\n" => 'v']];
    ...
    /**
     * The check is now a character-set test (strspn), which — unlike the '+' quantifier it replaced
     * — accepts the EMPTY string by construction: strspn('') === strlen('') === 0. The empty key is
     * reachable ($data[''] = ..., a CSV with an unnamed header column, json_decode of {"":1}), and
     * an empty backtick-quoted identifier is a syntax error, so the guard is explicit. This pins the
     * hole the fix itself could have opened.
     */
    public function testPrepareRejectsTheEmptyAutoDerivedColumnNameThatABareCharacterSetCheckWouldAccept(): void {
        $data = [['' => 'v']];
    ...
    /**
     * FINDING (fixed): the auto-generated ON DUPLICATE KEY UPDATE clause was built from the column
     * list AFTER trim() had stripped the caller's backticks off it, so an explicit
     * "`order`,`key`" produced a correctly quoted INSERT clause and an UNQUOTED update clause:
     *
     *     INSERT INTO t (`order`,`key`) ... ON DUPLICATE KEY UPDATE order=VALUES(order),key=VALUES(key)
     *
     * "order" and "key" are reserved words, so that is a syntax error — on precisely the columns
     * whose quoting the caller had supplied to avoid one. Quoting an explicit $insertFields was the
     * documented escape hatch, and using it broke the statement.
     */
    public function testPrepareKeepsExplicitInsertFieldsQuotingInTheAutoGeneratedUpdateClause(): void {
        $data = [['order' => 1, 'key' => 'k']];

        $sql = SQL::prepareInsertOrUpdateMySQL($data, 't', '`order`,`key`');

        self::assertSame(
            "INSERT INTO t (`order`,`key`) VALUES (1,'k') "
                . 'ON DUPLICATE KEY UPDATE `order`=VALUES(`order`),`key`=VALUES(`key`)',
            $sql
        );
    }

    /** Padding around a quoted explicit column must not survive into the generated update clause. */
    public function testPrepareTrimsOnlyWhitespaceFromExplicitColumnsWhenGeneratingTheUpdateClause(): void {
        $data = [['id' => 1, 'order' => 2]];

        $sql = SQL::prepareInsertOrUpdateMySQL($data, 't', ' id , `order` ');

        self::assertSame(
            'INSERT INTO t ( id , `order` ) VALUES (1,2) '
                . 'ON DUPLICATE KEY UPDATE id=VALUES(id),`order`=VALUES(`order`)',
            $sql,
            'the INSERT clause stays the raw caller text; the generated clause is trimmed but keeps quoting'
        );
    }
    ...
    /**
     * FINDING (fixed): the non-array guard was added to the column-derivation path only. The
     * formatting loop kept `if (!is_array($row) && !is_object($row)) continue;`, which waves an
     * OBJECT row straight through to the default formatter — and `$row[$column] ?? null` on a
     * stdClass is an uncaught \Error ("Cannot use object of type stdClass as array"), out of a
     * method whose documented failure mode is \InvalidArgumentException. An object row is ordinary:
     * PDO::FETCH_OBJ, mysqli fetch_object(), json_decode() without the assoc flag.
     */
    public function testPrepareRejectsObjectRowInsteadOfRaisingAnUncaughtError(): void {
        $data = [['id' => 1], (object) ['id' => 2]];
    ...
    /**
     * The other half of the same guard, and the reason it throws rather than skipping: a scalar row
     * was skipped by `continue` AND still counted into $processedRows, so it was sliced out of the
     * by-ref $data while the call returned a statement — the row's data was gone and the caller was
     * told the batch succeeded. Silent loss is strictly worse than the \Error it sat next to.
     */
    public function testPrepareRejectsScalarRowInsteadOfSilentlyConsumingIt(): void {
        $data = [['id' => 1], 'not-a-row'];
    ...
    public function testPrepareLeavesDatasetUntouchedWhenARowIsNotAnArray(): void {
        $data = [['id' => 1], (object) ['id' => 2], ['id' => 3]];
        $original = $data;

        try {
            SQL::prepareInsertOrUpdateMySQL($data, 't', 'id', 'id=VALUES(id)');
            self::fail('a row that is not an array must be refused');
        } catch (\InvalidArgumentException) {
            // expected
        }

        self::assertEquals($original, $data);
        self::assertCount(3, $data, 'the already-formatted first row must not be consumed either');
    }
